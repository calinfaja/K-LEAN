...
setup_ollama() {
    log_info "Configuring Ollama (local) provider..."
    log_info "No API key needed for local Ollama"

    # Create .env file (empty, just for consistency)
    cat > "$CONFIG_DIR/.env" << EOF
# K-LEAN LiteLLM Environment Variables - Ollama
# Generated by setup-litellm.sh
...
EOF
    ...
    # Copy config template
    if [ -f "$REPO_DIR/config/litellm/ollama.yaml" ]; then
        cp "$REPO_DIR/config/litellm/ollama.yaml" "$CONFIG_DIR/config.yaml"
    elif [ -f "${HOME}/.config/litellm/ollama.yaml" ]; then
        cp "${HOME}/.config/litellm/ollama.yaml" "$CONFIG_DIR/config.yaml"
    fi

    log_warn "Make sure Ollama is running: ollama serve"
    log_warn "Pull models: ollama pull mistral-nemo deepseek-r1:14b neural-chat"
    log_success "Ollama configuration created"
}
...
select_provider() {
    ...
    if [ -z "$provider" ]; then
        ...
        echo "  4) Skip setup (use existing config)"
        echo ""
